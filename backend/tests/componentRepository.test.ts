...
describe.skipIf(!reachable)("componentRepository (integration)", () => {
  ...
  it("creates a standalone component with no scan (scan_id null)", async () => {
    const component = await componentRepository.createComponent(baseInput, tx);
  ...
  it("creates a component linked to a scan via DETECTED", async () => {
    ...
    const component = await componentRepository.createComponent({ ...baseInput, scanId: scan.id }, tx);
  ...
  it("creates a batch containing one of the newly-added component types (relay) and preserves it exactly, not 'unknown'", async () => {
    ...
    const readBack = await componentRepository.getComponentById(components![0]!.id, tx);
    expect(readBack?.type).toBe("relay");
  });

  it("returns null from createDetectionBatch when the scan doesn't exist", async () => {
    const components = await componentRepository.createDetectionBatch(
      "does-not-exist",
  ...
  it("returns null when getting a component that doesn't exist", async () => {
    const component = await componentRepository.getComponentById("does-not-exist", tx);
    expect(component).toBeNull();
  });

  it("reports componentExists correctly", async () => {
    const component = await componentRepository.createComponent(baseInput, tx);
    await expect(componentRepository.componentExists(component.id, tx)).resolves.toBe(true);
    await expect(componentRepository.componentExists("does-not-exist", tx)).resolves.toBe(false);
  });
  ...
  it("gets a component with its full test-result history attached", async () => {
    ...
    const detail = await componentRepository.getComponentById(component.id, tx);
  ...
  it("lists components filtered by type and status", async () => {
    await componentRepository.createComponent({ ...baseInput, type: "resistor" }, tx);
    await componentRepository.createComponent({ ...baseInput, type: "led" }, tx);

    const resistors = await componentRepository.listComponents({ type: "resistor" }, tx);
    expect(resistors.every((c) => c.type === "resistor")).toBe(true);

    const notTested = await componentRepository.listComponents({ status: "not_tested" }, tx);
  ...
  it("updates a component's fields and re-parents it to a different scan", async () => {
    ...
    const component = await componentRepository.createComponent({ ...baseInput, scanId: scanA.id }, tx);
  ...
  it("returns null when updating a component that doesn't exist", async () => {
    const updated = await componentRepository.updateComponent("does-not-exist", baseInput, tx);
  ...
  it("deletes a component and cascades its test results", async () => {
    ...
    const deleted = await componentRepository.deleteComponent(component.id, tx);
    expect(deleted).toBe(true);

    const after = await componentRepository.getComponentById(component.id, tx);
    expect(after).toBeNull();
  });

  it("returns false when deleting a component that doesn't exist", async () => {
    const deleted = await componentRepository.deleteComponent("does-not-exist", tx);
